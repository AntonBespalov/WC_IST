...
- Изменение `duty_used` **SHALL** быть ограничено настраиваемым slew-rate (ограничитель скорости изменения). Единицы и допустимые границы параметра фиксируются в реализации и протоколе сервиса.
...
- Источник **SHALL** обеспечивать наблюдаемость: фиксировать момент применения duty через `DBG_PWM_APPLY`, а для аварий — `BKIN_RAW` и/или вход в софт-обработчик (см. `docs/PROJECT_CONTEXT.md` / раздел 7).
...
- Ошибка синхронизации (CCR обновляется не на update event) → глитчи на ШИМ; выявляется осциллографом по `DBG_PWM_APPLY` vs `PWM_OUT`.
...
- Осциллограф: `PWM_OUT` + `DBG_PWM_APPLY` — подтверждение применения duty строго на границе периода.
