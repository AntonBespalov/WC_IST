...
- медленный домен: обработка команд/параметров с периодом **1 мс** (CAN) и сервисные функции в FreeRTOS.
...
### 6.1 Fast/Slow sync (sequence counter снапшот)
...
Рекомендуемый паттерн: **двухфазная запись + sequence counter** (lock-free snapshot).

Writer (slow-домен, `control_slow_step()`):
1) `cmd_seq++` → `cmd_seq` становится нечётным (маркер “идёт запись”).
2) копирование структуры команды `cmd` целиком (все поля).
3) `cmd_seq++` → `cmd_seq` становится чётным (маркер “запись завершена”).

Reader (fast-домен, `control_fast_step()`):
1) прочитать `seq_start = cmd_seq`; если нечётный → запись в процессе, повторить попытку.
2) прочитать структуру команды в локальный `cmd_snapshot`.
3) прочитать `seq_end = cmd_seq`.
4) если `seq_start == seq_end` и он чётный → снапшот согласован (writer не вмешался).
5) если снапшот не удаётся получить за ограниченное число попыток → deny-by-default (safe output) с диагностикой.

Примечание: этот паттерн не добавляет блокировок в fast-домен и даёт детерминированный верхний предел времени (число попыток фиксировано).
