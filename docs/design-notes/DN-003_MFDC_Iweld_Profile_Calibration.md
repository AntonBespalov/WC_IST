...
- Без обязательного `profile_id_active` в телеметрии возможен “тихий” reject выбора профиля по интерфейсу ТК (если ТК ошибся `profile_id` или попытался применить не в IDLE).
