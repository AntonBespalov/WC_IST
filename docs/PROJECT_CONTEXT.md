...
Синхронизация: основные факты ниже приведены в соответствие с каноническими документами `docs/` (см. `docs/DOCS_INDEX.md`) по состоянию на **2026-01-31**.

---

## 1) Краткое описание системы
- Изделие: Источник сварочного тока (контроллер MFDC/силовой части)
...
- Сервисный режим `ManualDuty` (ручная скважность) для отладки ШИМ: duty 10–50%, slew-rate (настраиваемый), применение строго по периоду через preload, timeout связи 20 мс (см. `docs/design-notes/DN-002_MFDC_ManualDuty_Service_Mode.md`)
