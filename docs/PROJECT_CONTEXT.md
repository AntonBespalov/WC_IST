...
- Обмен с ТК:
  - Период команд: **250 мкс** (4 кГц)
  - Ответ источника: **периодически 250 мкс** (4 кГц) (статус + измерения + fault/limit слова)
  - Применение уставки в fast loop: используется **последняя валидная команда**, защёлкнутая на границе периода PWM (изменения внутри периода не влияют до следующего периода PWM)
...
- Внешние АЦП по SPI:
  ...
  - **2× AD7606** (многоканальный АЦП для “медленных” каналов), подключение: **SPI3** и **SPI4**
    - AD7606 #1 (SPI3): измерения тока/напряжения “входа”
      - ток с первичной обмотки сварочного трансформатора
      - входной ток: фазы **A, B, C**
      - входное напряжение: **A-B, B-C, A-C**
    - AD7606 #2 (SPI4): измерения температур
      - датчики температуры силового модуля **SEMiX252GB12** (4 шт.)
      - датчик температуры выпрямителя
      - датчик температуры внутри корпуса
- Измеряемые величины:
  - Ток сварочного трансформатора: аппаратный выбор источника сигнала между
    - собственным датчиком тока трансформатора (если установлен),
    - катушкой Роговского (устанавливается на обмотку трансформатора, если собственного датчика тока нет),
    далее → интегратор → AD7380 CH1 (`I_weld`)
  - Напряжение вторички: → AD7380 CH2 (`U_weld`)
  - Доп. каналы (температуры, токи, питания): AD7606 (см. выше)
...
- Буферизация/согласование выходов ШИМ: **CD4049UBDR** (между TIM1 и входами драйверов)
- Входы/выходы управления драйвера (заполнить по схеме):
  - PWM IN: TIM1_CHx/CHxN → CD4049UBDR → входы драйверов
...
- Debug-пины (должны иметь тест-пойнты): **2 шт.**
  - DBG0 (настраиваемый): программно выводится выбранное событие
  - DBG1 (настраиваемый): программно выводится выбранное событие
  - Рекомендуемые события для вывода (по выбору прошивки): шаг управления, старт выборки/транзакции SPI, готовность данных (DMA complete), момент применения CCR/ARR, вход в софт-обработчик аварии.
- Обязательные тест-пойнты силового домена:
  - BKIN_RAW — реальная линия аварии в TIM1
  - PWM_OUT — реальный выход TIM1_CHx/CHxN
- Внешняя память для буферизации “цифрового осциллографа”/логирования: **PSRAM APS6404L-3SQR-SN (QSPI/QUADSPI)** (см. `docs/design-notes/DN-005_QSPI_PSRAM_Oscilloscope_Buffer.md`).
- Логгирование/осциллографирование: см. `docs/logging/LOGGING_GUIDE.md` (формат Record, pre/post trigger, ресинхронизация по magic `0xA55A`).
- Дефолтный режим: поток логов/осциллографирования **сразу на ПК** через USB-UART, с промежуточной буферизацией в SRAM; запись в PSRAM включается при необходимости (критерии — TBD).
- Логирование/буферизация в память (если задействуется PSRAM):
...
- Даташиты: `docs/datasheets/README.md`
