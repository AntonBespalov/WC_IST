...
**Плата источника (ИСТ / УСПФ)**  
Та же сущность, что и «Источник». В документах проекта и переписке может также обозначаться как «ИСТ», «УСПФ» или «плата источника сварочного тока».
...
Неволатильная память с быстрым чтением/записью; в проекте может рассматриваться как опция для record/replay и устойчивого хранения логов (если будет принято решение).

**PSRAM (Pseudo-Static RAM)**

Энергозависимая внешняя память (как “большая RAM”), обычно подключаемая по QSPI/OSPI; в проекте используется для буферизации данных “цифрового осциллографа” (см. `docs/PROJECT_CONTEXT.md` / раздел 7 и `docs/design-notes/DN-005_QSPI_PSRAM_Oscilloscope_Buffer.md`).

**CY15B104Q-PZXI**

Микросхема FRAM, подключаемая по QSPI (если будет применяться в проекте; детали режимов и критериев переключения — TBD).

**APS6404L-3SQR-SN**

Микросхема PSRAM, подключаемая по QSPI/QUADSPI; используется как внешняя память для буферизации осциллографирования по USB-UART.
...
Единица обмена в интерфейсе Источник ↔ ТК (например, EtherCAT PDO; legacy/fallback: CAN-кадр), содержащая команды или статус.

**EtherCAT**  
Промышленный Ethernet для real-time обмена. В проекте используется как штатный интерфейс Источник ↔ ТК через COMX 100CA-RE.

**PDO (Process Data Object)**  
Циклические данные EtherCAT, которыми обмениваются мастер и устройство. В проекте RxPDO несёт команды ТК, TxPDO — статус/измерения источника.

**RxPDO / TxPDO**  
RxPDO — данные “мастер → устройство” (команда ТК). TxPDO — данные “устройство → мастер” (обратная связь/телеметрия).
...
**Bus-off (CAN)**  
Состояние CAN-контроллера при серьёзных ошибках шины. Актуально только для legacy/fallback режима обмена с ТК по CAN.

**USB-UART**  
Интерфейс UART, физически подключённый к ПК через USB-UART преобразователь (обычно “виртуальный COM-порт” на ПК).

**PCcom4 (PCcom 4.02 / PCCOM 4.02)**  
Прикладной протокол обмена с фреймингом по `0xFF`, полем длины и CRC16. В проекте используется для обмена “плата ↔ ПК” по USB-UART: настройка параметров, осциллографирование/логирование и сервисный режим отладки (в т.ч. обёртка CAN-кадров). См. `docs/protocols/PCCOM4.02.md`.
...
**Record (лог‑запись)**  
Сериализованный заголовок + payload; заголовок начинается с magic `0xA55A` (LE). См. `docs/logging/LOGGING_GUIDE.md`.

**Resync (ПК)**  
Поиск валидного заголовка Record в байтовом потоке по magic, чтобы восстановить выравнивание.

**SPSC (single‑producer/single‑consumer)**  
Очередь “один producer — один consumer”; важен единый контекст записи и чтения.
