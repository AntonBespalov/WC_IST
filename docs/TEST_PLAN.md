...
Дополнительно для логгера (`Fw/logging`):
- pack/unpack заголовка Record + проверка magic `0xA55A`
- `logging_spsc`: push/pop и граничные условия
- `logging_capture_sram`: `read_chunk` только после `STOPPED`
...
Дополнительно для логгера (если включён):
- jitter fast loop при включённом логгере (DBG0/DBG1)
- `read_chunk` после `STOPPED` (корректные длины/смещения)
- `logging_tx_scheduler`: PDO всегда приоритетнее LOG при ограниченном бюджете
...
- лог-стрим: ресинхронизация по magic `0xA55A` при обрезанной записи / границе pretrigger
