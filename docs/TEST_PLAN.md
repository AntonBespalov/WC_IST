...
- `RAMP_START → I_ref_used=0` и `PWM_OUT OFF` (t_ramp и задержка выключения)
...
- controlled ramp down для C1: t_ramp 10–20 мс (дефолт 10 мс) и эскалация hard-timeout
...
R2) Потеря команд ТК > N мс: soft-timeout → C1 ramp 10–20 мс (дефолт 10 мс) → OFF; hard-timeout → немедленный OFF + latch; отчёт в статус.  
...
R7) Overrun критического цикла (fast-loop: шаг управления не завершён до следующего периода PWM): фиксируется и приводит к немедленному OFF + latch.  
...
- C0 (BKIN) во время C1-ramp
- Перегруз P1 (EtherCAT‑лог‑стрим) → дропы в `P1`, `P0` не деградирует
- Конфликт capture‑сессий: попытка армировать источник, уже занятый активной сессией → отказ `CaptureArm`
...
- `SvcTs v1`: присутствие в Device→Host сообщениях `Node=0x03/0x06`, корректная обработка wrap-around
- `Scope.CaptureMeta`: согласованность `total/pre/post` с фактическим окном, корректный `POST_REDUCED`/`NO_SPACE`
- `Scope.StreamControl` dataset `0x03`: включение/выключение EtherCAT‑лог‑стрима, отсутствие стрима при OFF
- `Scope.Data` dataset `0x03`: `DataLen=73`, payload `SvcTs v1 + Rx16 + Tx48`, корректный `op=0x13`
- multi‑session capture: хост задаёт `session_id`, проверка `MAX_ACTIVE_SESSIONS` и запрета двух активных сессий на один источник
...
- RX DMA circular zero-copy: корректность индекса по NDTR (wrap-around), отсутствие memcpy в ISR, переполнение → flush + resync по PREAMBLE, частые overflow → статус degraded (без fault)
- если включена буферизация во внешнюю память (QSPI PSRAM): измерить throughput запись/чтение, проверить ошибки/timeout QSPI и поведение при переполнении/деградации (без влияния на fast loop)
- измерения: jitter по `DBG0/DBG1` (сконфигурировать, например, `CTRL_TICK`), worst-case длительность шага управления при включённом обмене по USB-UART
- проверка `SvcTs v1`: монотонность `ts_us`, согласованность `pwm_period_cnt` с периодом TIM1, корректный wrap-around
- EtherCAT‑лог‑стрим: при включении `dataset_id=0x03` P0/P1/P2 приоритеты сохраняются, `P0` не деградирует, дропы `P1` отражаются счётчиками

Сервисный режим “CAN over PCcom4” (отладка):
- приём “упакованных” legacy CAN-кадров *(устар.)* от ПК и их выдача в канальный слой без требований 1 мс (допускаются задержки мс)
- негативные кейсы: некорректный формат CAN-пакета внутри PCcom4, бурст пакетов, out-of-order, дубли

Сервисный режим `ManualDuty` (ручная скважность, отладка ШИМ):
- вход только из IDLE/safe state, регулятор тока выключен, но safety-инварианты активны (см. `docs/design-notes/DN-002_MFDC_ManualDuty_Service_Mode.md`)
- проверка диапазона duty 10–50% и настройки slew-rate (валидаторы на приёме)
- проверка slew-rate default 2‰/период и ограничения диапазона 1–10‰/период
- таймаут сервис-связи 20 мс по интерфейсу активации ⇒ safe state (на стенде измерить реакцию + убедиться, что PWM выключен)
- наблюдаемость: `PWM_OUT` vs `DBG0/DBG1` (сконфигурировать, например, `PWM_APPLY`), плюс логи/trace `duty_used` и флаг активного режима

Сервисный режим `RUN_SELFTEST` (factory/production диагностика):
- команда доступна только в `IDLE` при подтверждённом `PWM OFF` (gating)
- выполнение self-test **не должно** влиять на fast loop (нет блокировок ISR, счётчики overrun не растут)
- минимальные проверки: ADC noise floor (I/U), VREFINT, температуры (обрыв/КЗ/диапазон), (опционально) safety-path/BKIN флаг
- негативные кейсы: попытка запуска self-test в `WELD/ARMED` → команда отвергается; “бесконечный” self-test → таймаут/ошибка с возвратом в safe state
...
- t_ramp: `RAMP_START → I_ref_used=0` и `PWM_OUT OFF`
