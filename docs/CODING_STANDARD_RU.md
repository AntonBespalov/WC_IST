...
- **все поля структур**: у каждого поля должно быть краткое описание назначения.
- **все элементы перечислений (`enum`)**: у каждого элемента должно быть краткое описание смысла.
- **числовые величины** (поля структур, параметры и локальные переменные, где это важно для понимания):
  должна быть явно указана размерность/единицы измерения (например, `[А]`, `[В]`, `[с]`, `[А/с]`, `[шаги]`, `[отн. ед.]`).
- **нетривиальные функции** (длинные, с safety/RT ветками, с несколькими этапами вычисления):
  должны иметь “скелет алгоритма” в виде коротких шагов (`Шаг 1`, `Шаг 2`, ...), чтобы порядок действий читался сверху вниз.
- **все функции** (включая `static`) должны иметь Doxygen-описание минимум с блоками:
  `@brief`, `@param` (для каждого аргумента), `@retval` (или `@return`).
- **публичный API в заголовках (`.h`)**: Doxygen должен быть на объявлении
  функции и быть достаточным для использования API без чтения `.c`.
...
- выносить отдельные подалгоритмы в `static` helper-функции, если это уменьшает вложенность и когнитивную нагрузку.
- использовать предметные имена локальных переменных (по смыслу домена и роли в алгоритме), избегать “общих” имён вроде `tmp`, `val`, `flag` без контекста.
- для сложных/safety-значимых функций добавлять расширенные блоки:
  `@details`, `@pre`, `@post`, `@note`, `@warning` (по уместности).
...
- оставлять числовые величины без понятной размерности, если из контекста это неочевидно.
- смешивать в одной большой функции несколько этапов без явных границ, если из-за этого теряется читаемость и проверяемость.

### 1.3 Рекомендуемый формат
- Для всех функций: Doxygen‑стиль (минимум `@brief/@param/@retval`).
- Для complex/safety функций: дополнительно `@details/@pre/@post/@note/@warning`.
- Для публичных функций: размещать Doxygen у объявления в `.h`
  (а не только у определения в `.c`).
...
Профиль форматтера CDT в проекте:
- исходный профиль: `tools/eclipse/EclipseCodeStyle.xml`
- project-specific настройки Eclipse/CDT: `.settings/org.eclipse.cdt.core.prefs`
- синхронизация профиля в prefs: `tools/eclipse/sync_cdt_formatter_prefs.ps1`
...
- Если обновлён `tools/eclipse/EclipseCodeStyle.xml`, нужно пересинхронизировать `.settings/org.eclipse.cdt.core.prefs` скриптом `tools/eclipse/sync_cdt_formatter_prefs.ps1` и закоммитить оба файла.
