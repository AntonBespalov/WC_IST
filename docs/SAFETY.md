...
- CAN bus-off / ошибки линии (включая случаи, когда связь “логически” невалидна)
...
- Кто инициирует: ТК (команда reset_fault) / локальная кнопка / power-cycle (TBD).
- Условия снятия:
...
- Связь с ТК валидна (по таймауту и правилам seq/CRC).
...
- Нет активного `BUS_OFF_ACTIVE`/bus-off, либо он обработан по политике восстановления.

(Конкретные пороги/окна времени — фиксируются в `PROJECT_CONTEXT.md` и/или `docs/protocols/PROTOCOL_TK.md`.)

Примечание по bus-off recovery:
