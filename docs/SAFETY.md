...
- Потеря обновления EtherCAT PDO / link down (если штатный интерфейс ТК — EtherCAT; для legacy CAN — bus-off/ошибки линии, включая случаи, когда связь “логически” невалидна)
- Устойчиво невалидные измерения (timeout/stuck/sat/CRC)
- Overrun критического цикла
- Некорректная команда (режим/уставка вне допустимого диапазона)

Реакция:
- Перейти в controlled stop или fault (по политике ниже).
- Обязательное запрещение сварки: PWM off + DRV_EN off (если возможно).
- В зависимости от причины: latch или запрет запуска до восстановления условий.
...
### 5.1 Latch (защёлка)
- C0: всегда latch.
- C1: по политике причины (например, overrun может быть latch; единичная “мягкая” ошибка связи — может быть не latch).
...
- Если включён legacy CAN: нет активного `BUS_OFF_ACTIVE`/bus-off, либо он обработан по политике восстановления.

(Конкретные пороги/окна времени — фиксируются в `PROJECT_CONTEXT.md` и `docs/protocols/PROTOCOL_TK_ETHERCAT.md`; legacy CAN — в `docs/protocols/PROTOCOL_TK.md`.)

Примечание по legacy CAN bus-off recovery:
- По умолчанию **не использовать** hardware ABOM (чтобы исключить неконтролируемые попытки восстановления и “спам” в шину).
- Восстановление CAN — программное, с backoff/ограничением частоты попыток (см. `docs/protocols/PROTOCOL_TK.md` / раздел 5).
