...
Тайминги (Draft 0.1; совпадают по целевым значениям с `docs/protocols/PROTOCOL_TK.md`):
- Период команд ТК: **1 мс**
- Временные требования:
  - Максимальная задержка применения команды: **≤ 1 мс** (≤ 1 периода) (цель)
...
Примечание (отладка):
- При стендовой отладке без реального EtherCAT master взаимодействие может имитироваться с ПК по USB‑UART (PCcom4) (см. `docs/protocols/PCCOM4.02_PROJECT.md` / 3.2). Тайминги 1 мс в этом режиме не гарантируются.
...
- bit1 `CMD_REJECTED`: 1, если **последний** принятый на вход `CMD_WELD` был отвергнут валидатором. Сбрасывается при **следующем** успешно применённом `CMD_WELD`.
...
- bit8 `SEQ_GAP_DETECTED`: 1, если **последний применённый** `CMD_WELD` имел gap по `seq` и был применён как “вперёд с пропуском”. Сбрасывается при применении кадра с `delta==1`.
...
- В статусе отдавать агрегированные по периоду значения (`I_per`, `U_per`).
