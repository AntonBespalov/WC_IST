...
- `docs/protocols/tk_protocol.dbc` — машинно‑читаемое описание (для CANalyzer/SavvyCAN/cantools и т.п.). До фиксации байтовой раскладки здесь используется “байтовая” разбивка payload (b00..b63).
...
- `max_slew_rate_mA_ms` (лимит dI/dt): **int32_t**, единица **mA/мс** (**обязательное поле**)
- `limits` (опционально): `I_max`, длительность импульса и др. (TBD)
- `fault_reset` (опционально): запрос на recovery (0/1) (Draft 0.2; применимо только в `state=FAULT` и только по политике `docs/SAFETY.md`)
- `flags` (битовые флаги): **u8**, Draft 0.2 = 0 (резерв)
- `crc` (если используется поверх CAN payload): Draft 0.2 = 0 (не используется; целостность обеспечивает CRC CAN)

### 3.2 Правила валидации
- Диапазоны уставок: `I_ref_cmd` в пределах `0 … I_ref_max_mA` (TBD, фиксируется проектом); единица mA фиксирована.
- `max_slew_rate_mA_ms` MUST присутствовать в кадре; допустимый диапазон значений — TBD (фиксируется проектом).
- Недопустимые комбинации: `flags != 0` → кадр не применять (Draft 0.2).
- Политика `seq`:
...
- CRC/контроль целостности: для CAN FD отдельный payload CRC не используется; целостность обеспечивает CRC CAN. `crc != 0` трактуется как `CMD_INVALID` (Draft 0.2).

### 3.3 Семантика команд
- Что означает `enable=1`: разрешение на сварку при выполнении условий safety/gating (см. `docs/SAFETY.md`).
- Что означает `enable=0`: запрет сварки; PWM OFF не позже чем через **2 периода PWM** (Draft 0.2). Controlled stop (спад `I_ref_used`) допускается, но всегда ограничен `soft-timeout/hard-timeout` политикой.
...
- `fault_code` (опционально): enum последней причины (TBD; в Draft 0.2 достаточно `fault_word` + `status_word`)
...
- `P_per` (средняя мощность за период, `mean(I*U)`): **u16**, масштаб **1 Вт/LSB** (Draft 0.2, опционально)
- `temp`/прочее: TBD
...
- `fault_code`: опциональный enum “последней причины” (Draft 0.2 оставляет как TBD; приоритет у `fault_word`/`status_word`).
...
### 4.3 Сообщение `FAULT` (CAN-ID `0x010`)

Назначение: немедленное уведомление о критичной аварии (в дополнение к `FB_STATUS` 1 кГц).

Минимально рекомендуемая полезная нагрузка (TBD, фиксируется проектом):
- `seq_applied` (последняя применённая команда, если релевантно)
- `state=FAULT`
- `fault_word`, `fault_code`
...
Если используется `SERVICE_RESP`, его формат фиксируется проектом (TBD), но **минимум** должен включать: признак активности `ManualDuty`, `duty_used_permille`, возраст сервиса (мс).
