...
| Операция | `Op` | `DataLen` | `FrameBytes` | Доступ | NVM | Формат/примечание |
|---|---:|---:|---:|---|---|---|
| Установить скважность ШИМ | `0x01` | 4 | 13 | чтение/запись | нет | `float` (параметр не должен сохраняться во flash) |
| Провести самодиагностику | `0x02` | 1 | 10 | чтение/запись | нет | `u8`: `0x00`=выключить, `0x01`=включить |
| Результат самодиагностики | `0x03` | 20 | 29 | сообщение | нет | размер и формат данных будут уточнены |
