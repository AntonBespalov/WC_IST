...
- Выбранный вариант: разделить наблюдаемость на два потока и реализовать буферизацию через SPSC по контекстам исполнения:
  ...
  - Буферизация: вариант B (SPSC) — отдельные кольцевые буферы по контекстам (как минимум fast/ISR/task), а склейка/отправка/запись в PSRAM выполняются только в slow task.
  - Временная привязка: гибрид `fast_seq` + `timestamp_us` в каждой записи/метаданных блока.
    - `timestamp_us` — монотонная timebase в микросекундах (u32, wrap-around допустим) от отдельного free-running 32-бит таймера (TIM2/TIM5) с частотой 1 МГц; чтение допускается в fast/ISR как O(1) операция.
    - TIM1 работает в center-aligned; `TIM1.RCR=1`, чтобы Update/UEV происходил 1 раз на полный PWM-цикл (up+down), а не на каждый “разворот” счётчика.
    - `fast_seq` — номер периода PWM, формируемый аппаратно как счётчик Update/UEV (не через инкремент в ISR): 32-бит таймер-счётчик (TIM2/TIM5) в external clock mode от внутреннего триггера `TIM1 TRGO=Update` (конкретный ITR/маршрут подтверждается по RM для STM32G4).
    - (Опционально) для фазовой привязки “внутри периода” допускается добавлять `pwm_phase_ticks` как `TIM1.CNT` + направление (DIR) при событии записи/триггера; это поле не заменяет `timestamp_us`.
- Включение сервиса:
...
- Гибрид временных меток (`fast_seq` + `timestamp_us`) позволяет коррелировать домены и события на ПК без требования идеального “единого порядка” на МК.
...
- Негативные последствия/долг:
  - требуется реализовать и поддерживать таймбазу `timestamp_us` (переполнение, единицы измерения) и правила корреляции на ПК;
...
- Интерфейсы/форматы/контракты:
  - `Trace`: единый формат записи (заголовок: `timestamp_us`, `fast_seq`, `source_id`, `event_id`, `len`, далее `payload[len]`), политика drop и счётчики.
  - `Scope`: метаданные блока DMA (например `t0_us`, `fast_seq0`, `sample_count`, `channel_count`, `reason`, `capture_id`), а сырые выборки — как массивы фиксированного типа (например u16).
...
- On-target измерения (GPIO/осциллограф/trace):
  ...
  - подтвердить корректность `fast_seq` (1 инкремент на полный PWM-цикл) при штатной нагрузке и при worst-case нагрузке на IRQ (например UART flood), без зависимости от ISR;
...
- Follow-ups: TODO список задач (если есть)
  - Подтвердить по RM STM32G4 конкретный маршрут `TIM1 TRGO=Update` → ITR для выбранного таймера-счётчика `fast_seq` (TIM2/TIM5), и зафиксировать его в реализации/конфиге.
  - Зафиксировать правила корреляции на ПК при wrap-around `timestamp_us` (u32) и возможных потерях (drop) в буферах.
  - Зафиксировать схему синхронизации TIM1 (PWM/UEV/TRGO), таймбазы (`timestamp_us`/`fast_seq`) и выборок AD7380 (события, окна, DMA) отдельным документом (см. `docs/design-notes/DN-008_AD7380_Sampling_and_Timing_Sync.md`).
