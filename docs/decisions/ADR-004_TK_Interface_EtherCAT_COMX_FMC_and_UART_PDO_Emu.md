...
- Контекст проекта по доменам и safety: см. `docs/CONTEXT_SNAPSHOT.md` (fast loop = PWM период; slow loop = **250 мкс команды ТК**; аппаратный shutdown-path через TIM1 BKIN/BKIN2).
