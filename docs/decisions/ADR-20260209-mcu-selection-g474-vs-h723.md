...
- Ограничения/инварианты, которые нельзя нарушать (см. `docs/PROJECT_CONTEXT.md` / разделы 2, 4–6 и `docs/ARCHITECTURE.md` / P2–P3):
  ...
  - Контракт таймингов: шаг управления **1 раз на период PWM** (1–4 кГц) + обмен с ТК **250 мкс (4 кГц)** по EtherCAT PDO.
...
- Контракты интерфейсов/таймингов **не меняются** (см. `docs/PROJECT_CONTEXT.md`):
  ...
  - EtherCAT (COMX/FMC) к ТК: период **250 мкс (4 кГц)**; таймауты по политике.
...
- Замечания:
  - В предоставленном H723 экспортe **не отражены** FMC (под COMX/EtherCAT) и UART под PCcom4 (и legacy CAN FD, если он остаётся как fallback): при серьёзном рассмотрении H723 требуется донастроить CubeMX и проверить pin-матрицу под FMC + USART + debug (и при необходимости FDCAN).
