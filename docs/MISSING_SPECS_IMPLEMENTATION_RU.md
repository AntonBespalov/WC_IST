...
Область: документация в `docs/` (в первую очередь `PROJECT_CONTEXT.md`, `SAFETY.md`, `PROTOCOL_TK_ETHERCAT.md`, `PROTOCOL_TK.md`, `TEST_PLAN.md`).
...
### 1.4.1 PDO mapping и байтовая раскладка (P0)
Нужно: определить **RxPDO/TxPDO** (набор полей, размеры, порядок байтов, MUST=0 для reserved) и правила версионирования (`proto_ver`/совместимость).

Минимум:
- поля enable/stop/fault_reset (если применимо), `seq`, уставка тока/напряжения (единицы), статус/ошибки, `seq_echo`;
- диапазоны и валидаторы для всех полей (что делать при out-of-range);
- политика обработки `seq` (wrap-around, out-of-order, пропуски).

### 1.4.2 Тайминги и watchdog (P0)
Нужно: явно определить:
- целевой период обновления PDO (например, 1 мс) и допустимый джиттер;
- `LinkTimeoutMs` (soft/hard или один порог) и ожидаемую реакцию (controlled stop vs fault latch) по политике safety;
- что считается “валидной командой” (CRC/версии/полей reserved, свежесть).

### 1.4.3 “Атомарность” снимка process image через FMC (P0)
Нужно: определить контракт чтения/записи PDO:
- как гарантируем консистентность (seq-before/seq-after, двойной буфер, time-stamp, и т.п.);
- что делаем при неконсистентности/ошибке чтения (REJECT + счётчики + safe policy).

### 1.4.4 Поведение при отказах COMX/шины (P0)
Нужно: сценарии и реакция:
- COMX не отвечает/не инициализируется/зависает;
- прерывания/сигналы от COMX “пропали”;
- FMC/bus error (если применимо) или обнаружение некорректного process image.

Ожидаемое: система остаётся в safe state, сварка запрещена, статус/счётчики отражают причину.
