...
Критично: любой сбой связи с ТК (timeout обновления EtherCAT PDO / stale, битые данные, рассинхрон) должен приводить к **предсказуемому** безопасному поведению:
...
### 2.3. EtherCAT/PDO становится частью контура управления → нужен «control-grade» протокол
...
- как доказываете, что при нагрузке **EtherCAT/логированием** контур не разваливается
- какие у вас **worst‑case задержки/джиттер**

Поэтому стоит с самого начала:

- изолировать **real‑time контур** в коротком **ISR**
- все «красивости» (фильтры телеметрии, упаковка PDO/телеметрии, запись во внешнюю память) — строго **ниже по приоритету** и **без блокировок**
...
*(МК → ШИМ 1–4 кГц, EtherCAT, лог в память, выгрузка по RS‑485)*
...
2. **Быстрый «защитный монитор» в МК** *(высший приоритет, короткий, без EtherCAT/логики)*:  
   следит за током/напряжением/температурами/таймингами и гарантированно переводит в safe-state.

3. **Обычная прикладная логика**:  
   EtherCAT, фильтры «для ТК», запись в память, сервисные режимы.

Ключ: даже если (3) зависла или EtherCAT «залип» — (1)+(2) продолжают защищать.

### 1.3. Связь с ТК не должна быть «единственной тормозной педалью»
...
- лучше, чтобы «силовая часть разрешена» была **отдельным входом** от верхнего уровня безопасности (интерлок/реле/контактор), а не «разрешение по EtherCAT»
...
5. **Распределение ответственности**: кто выключает? драйвер? реле? ваш МК? ТК по EtherCAT?  
   Если единственная защита — команда по EtherCAT, это **красный флаг**.
...
- «Отключить при потере команд ТК» — зависит от сценария: если возможна опасная самоподача энергии → safety‑связанная; если максимум — брак точки → технологическая.
...
- отдельный путь отключения, **не зависящий** от связи с ТК и «обычного» ПО

### 3.2. Тайминги как часть безопасности и качества

- real‑time контур (измерение → регулятор → PWM update) не зависит от нагрузки EtherCAT/логирования
...
- **Protective** (защита железа; часто делается почти так же строго):  
  `Imax`, `dI/dt`, `Udc brown-out` ограничения, защита от насыщения, timeout связи с ТК.
