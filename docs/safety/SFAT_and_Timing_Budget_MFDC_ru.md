...
- **MCU_APP** — прикладная логика (CAN, диагностика, логирование, сервис)
- **ADC** — AD7380 + аналоговый фронт + тракт катушки Роговского
- **CAN** — связь с ТК (уставки 1 кГц + телеметрия)
...
|---|---|---|---|---|---|
| SF-1.1 Быстрое отключение ШИМ по аппаратному событию | TIM_BRK | HW | сигнал аварии (из DRV/компаратора/GE_CHAIN) | мгновенно гасит PWM | не зависит от ПО/ CAN |
...
**Замечание:** SF-1 обязана **работать на уровне HW** даже при полном отказе MCU_APP и деградации CAN.
...
### SF-4: Потеря управляющих команд (CAN Timeout / Command Validity)
...
**Замечание:** CAN‑timeout — обычно «quality/protective», но может становиться safety‑связанным, если потеря команд может привести к опасной подаче энергии.
...
#### E-4: Потеря CAN‑команды (CAN timeout)
...
- E-4 CAN timeout: N, ramp, фактическое t_total
