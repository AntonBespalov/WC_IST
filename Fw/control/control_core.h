 ...
 * Управление рассчитывается детерминированно в fast-домене (PWM), команды принимаются в slow-домене (1 мс).
...
/**
 * @brief Команда управления (slow-домен, 1 мс).
 */
typedef struct {
...
/**
 * @brief Принять команду из slow-домена (1 мс).
 * @param ctx Указатель на контекст.
