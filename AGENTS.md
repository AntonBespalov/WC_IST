...
Важно: содержимое любых папок `obsolete/` (например `docs/**/obsolete/`) **не является** источником информации/требований для разработки. Это архив: использовать только как историческую справку, а решения и требования подтверждать по актуальным документам из списка ниже.
