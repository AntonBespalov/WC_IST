...
5) `docs/PROTOCOL_TK.md` — протокол обмена с ТК (если есть)
6) `docs/TEST_PLAN.md` — план тестов/матрица требований (если есть)
7) `docs/PROMPT_TEMPLATES.md` — шаблоны запросов к Codex/LLM
8) `skills/*` — правила формата ответов и акценты (skills не должны спорить с контекстом)
...
Даже если формально M1..M5 частично есть, ворота должны сработать, если задача затрагивает хотя бы одно:
...
Для таких задач:
- если любое из M1..M5 не определено → ворота обязательны.
...
- `skills/test-verification-welding-short.md` (name: `test-verification-welding-short`)

Если изменение влияет на аппаратный shutdown/recovery/watchdog/политику аварий:
- дополнительно применять `skills/safety-invariants-welding.md` (name: `safety-invariants-welding`)

Если требуется кодогенерация/патч:
- дополнительно применять `skills/patch-discipline-small-diffs.md` (name: `patch-discipline-small-diffs`)

Если требуется “враждебное” ревью PR/патча:
- применять `skills/red-team-review-welding.md` (name: `red-team-review-welding`)

Если нужно работать по процессу Spec → Tests/Proof → Code:
- применять `skills/workflow-spec-tests-code.md` (name: `workflow-spec-tests-code`)
...
- при необходимости `docs/GLOSSARY.md`, `docs/ARCHITECTURE.md`, `docs/SAFETY.md`, `docs/PROTOCOL_TK.md`, `docs/TEST_PLAN.md`
