...
- STM32CubeIDE: если добавляешь новую папку с кодом — обновить include paths и `sourceEntries` в `.cproject` (см. `docs/HOW_TO_USE_RU.md` / раздел 5), иначе ломается индексация/переход к объявлению.
